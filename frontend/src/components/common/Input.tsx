...
import React, { useId } from 'react'; // Add useId
...
export const Input: React.FC<InputProps> = ({
  ...
  // Use useId for stable IDs between server and client
  const generatedId = useId();
  const inputId = providedId || `input-${label?.toLowerCase().replace(/\s+/g, '-') || generatedId}`;
