...
export default function ManagerStaffPage() {
  ...
  useEffect(() => {
    ...
    if (!authLoading && user?.role !== 'MANAGER') { router.push('/dashboard'); return; }
    if (!authLoading && isAuthenticated) fetchStaff();
  }, [isAuthenticated, authLoading, user, router]);

  const fetchStaff = async () => {
    setIsLoading(true);
    try {
      // Try JWT locations first, fall back to my-locations API
      let locId = (user as any)?.locations?.[0];
      if (!locId) {
        try {
          const locRes = await locationsService.getMyLocations();
          const locs = (locRes as any).data?.data || (locRes as any).data || [];
          locId = locs[0]?.id;
        } catch {}
      }

      const response = locId
        ? await staffService.getStaffByLocation(locId)
  ...
  return (
    <Layout>
      <div className="p-4 sm:p-6">
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 mb-6">My Team</h1>
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="sm:hidden space-y-3">
              ...
              {filtered.length === 0 && <p className="text-center text-gray-400 py-8 text-sm">No staff found</p>}
            </div>
